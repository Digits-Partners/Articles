...
A couple of algorithms maight be used to solve this optimization problem:
 ...
 - When the length of the sequence T<sub>y</sub> increases, Beam search results in some numerical underflow. This is due to the fact that we maximize the product of all the conditional probabilities. One way of fixing this numerical issue is to maximize the log likelihood (product of these probabilities, as the log is monotonically ascendent function), whitch is equivalent to maximizing the sum of the log probabilities. 
 
   In addition, because beam search is better doing with short sequences vs. longer sequences, we also need to normalize the maximization goal by (1/T<sub>y<sup>α</sup></sub>) where α ranges between 0 and 1. (See also Laplacian smoothing, applies more to naive bayesian approach. Use naive bayesian approach as a baseline for benchmarking):
...
<p align="center">
 ...
 - α<sup>\<i,j\></sup> is the amount of attention that y<sup>i</sup> should pay to the feature f<sup>j</sup>. For each state these weights must sum to 1.
   ...
   - To obtain e<sup>\<i,j\></sup>, train a small neural network to learn a function that maps the previous state s<sup>\<i-1\></sup> to f<sup>\<j\></sup>. This shows that the amount of attention given to f<sup>\<j\></sup> is mainly driven by the previous hidden state s<sup>\<i-1\></sup>.
