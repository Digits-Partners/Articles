...
2. Construct the embedding matrix (number of embedding vectors x number of words in the text corpus), to represent each word in the new embeding space by electing one of the following options:
   ...
   1. Learn a featurized representation from a large text corpus (up to hundred of billions of words), works well in machine translation. Some of the most popular algorithms are decribed bellow:
      ...
      3. There is computational challenges to the softmax classification, as it slows down when making prediction. This is because for each predection we need to sum up over all the embedding vectors of text corpus, whitch makes computations slower as the text corpus gets larger. The computaitonal cost in this case scales linearly to the text corpus size. 
      One solution to this problem is the hierarchical softmax (binary classifier) that iteratively classifies the context among two smaller groups of text corpus. These groups are structured as a binary tree (each parent node with two children) where the root is the text global corpus and leafs are(). the tree of the hierarchical softmax is not symetric in terms of node size. Infrequent words should be captured by deeper layers of the tree, whereas frequent words should found pretty much quicly to be classified. The computaiton cost in this case scales like the log(vocab size).
