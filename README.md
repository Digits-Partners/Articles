...
The Gated Recurrent Units (GRU) model is a variation of the RNN model, it has been designed to adress the lack of long term memory also known as "local influences problem". Technically speaking the GRU adds a new output for each layer unit called the "memory cell", denoted as "c<sup>\<t\><sub>". c<sup>\<t\><sub> will carry out any desired activation value from earlier layers up to the time step t where it is no more needed and will be replaced by the candidate cell memory value c-tilda<sup>\<t\><sub>. c<sup>\<t\><sub> is driven by an update-gate sigmoid function G<sub>u</sub> that will take 1 when the cell memory is replaced by  c-tilda<sup>\<t\><sub> and 0 otherwise.
   - c<sup>\<t-1\><sub> = a<sup>\<t-1\><sub>
   - c-tilda<sup>\<t\><sub> = tanh((W<sub>cc</sub>c<sup>\<t-1\></sup> + W<sub>cx</sub>X<sup>\<t\></sup>  + b<sub>c</sub>
   - G<sup>\<u\><sub> = sigmoid((W<sub>uc</sub>c<sup>\<t-1\></sup> + W<sub>ux</sub>X<sup>\<t\></sup>  + b<sub>u</sub>
   - c<sup>\<t\><sub>" = G<sup>\<u\><sub>c-tilda<sup>\<t\><sub> + (1-G<sup>\<u\><sub>)c<sup>\<t-1\><sub>
