...
- Broadly speaking speaking, the RNN processes input data from left to right. At each time step t, the RNN takes the input X<sup>\<t\></sup> and passes on the activation value a<sup>\<t\></sup> to the next step t+1, where :
   ...
   a<sup>\<t\></sup> = g<sub>1</sub>(W<sub>aa</sub>a<sup>\<t-1\></sup> + W<sub>ax</sub>X<sup>\<t-1\></sup>  + b<sub>a</sub>) and y^<sup>\<t\></sup> = g<sub>2</sub>(W<sub>ya</sub>a<sup>\<t\></sup> + b<sub>y</sub>). g<sub>1</sub> and g<sub>2</sub> may or may not be the same function (tanh, ReLU, ...);
