...
<p align="center">
 ...
 - α<sup>\<i,j\></sup> is the amount of attention that y<sup>i</sup> should pay to the feature f<sup>j</sup>. For each state these weights must sum to 1.
   ...
   - To obtain e<sup>\<i,j\></sup>, train a small neural network to learn a function that maps the previous state s<sup>\<i-1\></sup> to f<sup>\<j\></sup>. This shows that the amount of attention given to f<sup>\<j\></sup> ismainly driven by the previous hidden state s<sup>\<i-1\></sup>.
